...
class Optimizer:
    ...
    def clip_gradient(self, nabla):
        assert self.clipping_factor and self.clipping_factor > 0

        nabla_l2_norm = mpc_math.sqrt(sum(nabla * nabla))
        return util.if_else(nabla_l2_norm > self.clipping_factor, (nabla * self.clipping_factor)/nabla_l2_norm, nabla)

    def perturb_gradients(self, nabla):
        # assert self.clipping_factor and self.clipping_factor > 0
        assert self.sigma and self.sigma > 0

        sigma2 = self.sigma * self.sigma 

        from noise.discretegauss import sample_dgauss

        noise = sint.Array(len(nabla))
        for i in range(len(nabla)):
            noise[i] = sample_dgauss(sigma2)
    ...
    def run_by_args(self, program, n_runs, batch_size, test_X, test_Y,
        ...
        for arg in program.args:
            ...
            m = re.match('clip_factor(.*)', arg)
            if m:
                self.clipping_factor = int(m.group(1))
            else:
                self.clipping_factor = None
            m = re.match('sigma(.*)', arg)
            if m:
                self.sigma = int(m.group(1))
            else:
                self.sigma = None
        ...
        self.revealing_correctness = True
        ...
        print("revealing_corr = "+str(self.revealing_correctness))
        acc_first = False   # did this to easily print training accuracy
        ...
        def _(i):
            ...
            if not acc_first and self.print_accuracy and self.revealing_correctness:
            ...
            if test_X and test_Y:
                ...
                # print_ln("Program %s (%s): train_accuracy = %s (%s/%s) ; test_loss = %s ; test_accuracy = %s (%s/%s)\n", 
                #             program.name, 
                #             ('Emulate' if program.options.ring else 'Hemi'), 
                #             cfix(self.n_correct, k=63, f=31) / n_trained,
                #             self.n_correct, n_trained,
                #             loss,
                #             cfix(n_correct, k=63, f=31) / n_test, n_correct, n_test)
            ...
            if acc_first:
                ...
                def _():
                    ...
                    # self.reset()
                    # print_ln('reset after reducing learning rate to %s', self.gamma)
            if depreciation:
    ...
    def _update(self, i_epoch, i_batch, batch):
        for nabla, theta, delta_theta in zip(self.nablas, self.thetas,
            ...
            def _(base, size):
                ...
                # print_ln("Pre-Clipped Gradients:")
                # util.if_else(len(nabla_vector) > 10, print_ln("nabla = %s", nabla_vector.reveal()), print_ln("nabla = []"))

                ###### CLIPPING TAKES PLACE HERE ######
                if self.clipping_factor:
                    nabla_vector = self.clip_gradient(nabla_vector)

                # print_ln("Clipped Gradients:")
                # util.if_else(len(nabla_vector) > 10, print_ln("nabla = %s", nabla_vector.reveal()), print_ln("nabla = []"))

                if self.sigma:
                    nabla_vector = self.perturb_gradients(nabla_vector)

                # print_ln("Noisy Gradients Final: %s", nabla_vector.reveal())
