...
namespace osuCrypto
{
    ...
    public:
        ...
        struct AnyPOD
        {
            ...
            template<typename T, typename U = typename std::enable_if<std::is_trivial<T>::value, T>::type>
                operator T()
            {
                return mPrng.get<T>();
            }
    ...
    template<typename T>
    typename std::enable_if<std::is_trivial<T>::value, PRNG&>::type operator<<(T& rhs, PRNG& lhs)
    {
        lhs.get(&rhs, 1);
    }
