...
namespace osuCrypto {
    ...
    public:
        ...
        u8* data() const { return mData->data(); }
    ...
    inline BitReference BitVector::operator[](const u64 idx) const
    {
        ...
        return BitReference(mData->data() + (idx / 8), static_cast<u8>(idx % 8));
    }
    ...
    {
        return BitIterator(mData->data(), 0);
    }

    inline BitIterator BitVector::end() const
    {
        return BitIterator(mData->data() + (mNumBits >> 3), mNumBits & 7);
    }

    template<class T>
    inline span<T> BitVector::getArrayView() const
    {
        return span<T>((T*)mData->data(), (T*)mData->data() + (sizeBytes() / sizeof(T)));
    }

    template<class T>
    inline gsl::span<T> BitVector::getSpan() const
    {
        return gsl::span<T>((T*)mData->data(), (T*)mData->data() + (sizeBytes() / sizeof(T)));
    }
